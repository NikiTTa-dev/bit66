...
public class SoccerPlayerService: ISoccerPlayerService
{
    ...
    public async Task AddPlayerAsync(SoccerPlayerModel player)
    {
        ...
        if (await _unitOfWork.Commands.FindByNameAsync(playerEntity.Command.Name) is not Command command)
        {
        ...
        if (await _unitOfWork.Countries.FindByNameAsync(playerEntity.Country.Name) is not Country country)
        {
    ...
    public async Task<SoccerPlayerModel> EditPlayerAsync(SoccerPlayerModel player)
    {
        ...
        if (await _unitOfWork.Players.FindWithCommandAndCountryAsync(soccerPlayer.Id) is not SoccerPlayer playerEntity)
            throw new Exception("Player doesn't exist.");
        
        if (await _unitOfWork.Commands.FindByNameAsync(soccerPlayer.Command.Name) is not Command command)
        {
        ...
        if (await _unitOfWork.Countries.FindByNameAsync(soccerPlayer.Country.Name) is not Country country)
        {
